...
| **M1** | `fixmath` (sin/cos LUT, fixed-iteration sqrt/rsqrt, xorshift64, Vec2/Vec3/Mat), fixed 120 Hz timestep + render interpolation skeleton, `InputState`, FNV-1a frame hash, scripted-input harness | determinism test: two runs same seed+script → identical hash sequence; math property tests; no-libm grep test |
