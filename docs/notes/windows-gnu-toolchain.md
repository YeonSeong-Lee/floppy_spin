...
**Lesson: pin the gnu HOST toolchain (`stable-x86_64-pc-windows-gnu`); adding the gnu
TARGET to the msvc toolchain cannot link.**
...
- The gnu HOST toolchain bundles `rust-mingw` (binutils ld + full import-lib set) and is
  fully self-sufficient for pure-Rust crates. `rust-toolchain.toml` with
  `channel = "stable-x86_64-pc-windows-gnu"` pins it; the default build target is then
  already the ship target — plain `cargo build --release` is correct.
...
- rustup `--profile minimal` omits clippy/rustfmt — add them explicitly:
  `rustup component add clippy rustfmt --toolchain stable-x86_64-pc-windows-gnu`.
